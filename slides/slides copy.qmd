...
# Load all result tables once in setup so later slides can use shared objects.
rq3_group <- read.csv(
...
rq3_logo01 <- read.csv(
  "../rq3_finetune/results_logo_group_01.csv",
  stringsAsFactors = FALSE
)
...
orig_g01 <- rq3_group[rq3_group$group == "01", ]
logo01_local <- mean(rq3_logo01$local_nse)
logo01_global <- mean(rq3_logo01$global_nse)
logo01_ft <- mean(rq3_logo01$ft_nse)
logo01_ft_vs_local <- mean(rq3_logo01$delta_ft_minus_local_nse)
logo01_ft_vs_global <- mean(rq3_logo01$delta_ft_minus_global_nse)
logo01_global_vs_local <- mean(rq3_logo01$global_nse - rq3_logo01$local_nse)
logo01_ft_beats_local <- sum(rq3_logo01$ft_nse > rq3_logo01$local_nse)
logo01_ft_beats_global <- sum(rq3_logo01$ft_nse > rq3_logo01$global_nse)
logo01_global_beats_local <- sum(rq3_logo01$global_nse > rq3_logo01$local_nse)
```
...
- **Research gap:** Heterogeneous regional performance, untested transfer learning opportunity
...
**Coverage:** 533 basins across 18 geographic groups  
...
- **Architecture:** CudaLSTM (NeuralHydrology standard)
- **Global scope:** 531 CAMELS-US basins pre-trained
- **Evaluation:** 18 geographic groups (Daymet-based), 533 basins total
...
## Evaluation Coverage: 18 Groups, 533 Basins {data-section="Methodology"}
...
ggplot(cov_df, aes(x = group, y = n_basins)) +
  ...
  labs(
    x = "Geographic Group",
    y = "Number of basins",
    subtitle = sprintf("%d basins total across 18 Daymet-based groups", sum(rq3_group$n_basins))
  ) +
  theme_nh() +
...
rq3_long$group <- factor(rq3_long$group, levels = rq3_group$group[order(rq3_group$ft_nse_mean)])

ggplot(rq3_long, aes(x = group, y = nse, fill = model)) +
  geom_col(position = position_dodge(width = 0.75), width = 0.64) +
  scale_fill_manual(values = c("Local" = amber, "Global" = lmu_light, "Fine-tune" = lmu_blue)) +
  scale_y_continuous(limits = c(0, 0.92), expand = c(0, 0.01)) +
  labs(
    x = "Group (sorted by FT NSE)",
    y = "Mean NSE"
  ) +
  theme_nh() +
  theme(axis.text.x = element_text(size = 9))
...
ggplot(gain_df, aes(x = group)) +
  geom_col(aes(y = delta_ft_local, fill = "FT - Local"), width = 0.6) +
  geom_point(aes(y = delta_ft_global, color = "FT - Global"), size = 2.5) +
  geom_hline(yintercept = 0, linetype = "dashed", color = grey50) +
  scale_fill_manual(values = c("FT - Local" = lmu_blue)) +
  scale_color_manual(values = c("FT - Global" = green_ok)) +
  labs(
    x = "Group",
    y = "Delta NSE",
    color = NULL
...
ggplot(dk, aes(x = group)) +
  geom_col(aes(y = delta_ft_local, fill = "FT - Local"), width = 0.62) +
  geom_point(aes(y = delta_ft_global, color = "FT - Global"), size = 2.5) +
  geom_hline(yintercept = 0, linetype = "dashed", color = grey50) +
  scale_fill_manual(values = c("FT - Local" = lmu_blue)) +
  scale_color_manual(values = c("FT - Global" = green_ok)) +
...
## Leave-One-Group-Out (LOGO) Validation: Group 01 {data-section="Discussion"}

```{r rq3-logo-g01-compare, results='asis'}
logo_cmp <- data.frame(
  Setting = c("Original Global (includes group 01)", "Leave-One-Group-Out Global (excludes group 01)", "Local (LOGO run)", "Fine-tune (LOGO run)"),
  Mean_NSE = c(orig_g01$global_nse_mean, logo01_global, logo01_local, logo01_ft)
)

kable(logo_cmp, digits = 3, align = c("l", "c"), caption = "Group 01 mean NSE under original vs Leave-One-Group-Out setup")

cat(sprintf("\n\n**Differences (LOGO):** Fine-tune minus Local = %.3f, Fine-tune minus Global = %.3f, Global minus Local = %.3f", logo01_ft_vs_local, logo01_ft_vs_global, logo01_global_vs_local))
cat(sprintf("\n\n**Win counts (25 basins):** Fine-tune > Local = %d/25, Fine-tune > Global = %d/25, Global > Local = %d/25", logo01_ft_beats_local, logo01_ft_beats_global, logo01_global_beats_local))
```

- Leave-One-Group-Out validation removes data leakage concern by excluding group 01 from global pretraining.
- Even under LOGO, `Global > Local` on most basins and `Fine-tune` remains best.
- This supports the transfer-learning benefit beyond simple in-group exposure.
...
1. **Universal transfer benefit:**
   ...
   - FT > Global at **group level: 18/18 groups**
   - Not a single group where local-from-scratch outperforms fine-tuning
...
1. Across 18 groups and 533 basins, **global + fine-tune** consistently outperforms local-from-scratch.
