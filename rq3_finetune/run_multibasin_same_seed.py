...
basins = [b.strip() for b in BASIN_SOURCE.read_text(encoding="utf-8").splitlines() if b.strip()]
basins = basins[:N_BASINS]
if not basins:
    raise RuntimeError("No basins loaded from basins_global.txt")

base_global = newest("rq3_base_global", BASE_RUNS_ROOT)
print(f"Using global base: {base_global}")
print(f"Running {len(basins)} basins with fixed seed={FIXED_SEED}")
...
out_csv = WORK / f"results_multibasin_seed{FIXED_SEED}_{len(basins)}basins.csv"
...
summary = pd.DataFrame(
    {
        ...
        "n_basins": [len(basins), len(basins)],
        "seed": [FIXED_SEED, FIXED_SEED],
    }
)

summary_csv = WORK / f"results_multibasin_seed{FIXED_SEED}_{len(basins)}basins_summary.csv"
summary.to_csv(summary_csv, index=False)

print("\n=== MULTI-BASIN RESULTS SAVED ===")
print(result_df[["basin", "local_NSE", "finetune_NSE", "delta_NSE_ft_minus_local"]].to_string(index=False))
print("\n=== SUMMARY ===")
